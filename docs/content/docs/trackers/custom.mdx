...
description: Define your own statistics with factories or class adapters.
---

Trackers separate **definition** (behavior) from **state** (plain data). Author with a factory (`defineGameTracker` / `defineMoveTracker`) or a class adapter (`extends BaseMoveTracker` / `extends BaseGameTracker`).
...
Put class adapters in their **own module** with a **default export** for multithreaded analysis.

## Multithreading

Custom trackers need:

1. **Own module** with **default export** (class or factory object)
