...
Trackers separate **definition** (behavior) from **state** (plain data). Author with a factory (`defineGameTracker` / `defineMoveTracker`) or a class adapter (`extends MoveTracker` / `extends BaseGameTracker`).
