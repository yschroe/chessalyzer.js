...
Pass a function via the `analysis` option to `generateHeatmap(state, { analysis })`. The function is called for every square on the board.
...
const heatmapData = tileTracker.generateHeatmap(state, {
