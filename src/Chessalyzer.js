...
class Chessalyzer {
	...
	startBatch(path, bank = 0, refreshRate = 250) {
		return new Promise((resolve) => {
			...
			this.gameProcessor.processPGN(path, refreshRate).then((gameCnt) => {
				this.dataStore[bank] = JSON.parse(
					JSON.stringify(this.gameProcessor.board)
				);
				const t1 = performance.now();
				const tdiff = Math.round(t1 - t0) / 1000;
				const mps = Math.round(
					this.gameProcessor.board.cntMoves / tdiff
				);
				console.log(
					`${
						this.gameProcessor.board.cntMoves
