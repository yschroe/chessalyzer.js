import ChessPiece2 from './ChessPiece2';
	...
	constructor() {
		...
		for (let row = 0; row < 8; row += 1) {
			...
			for (let col = 0; col < 8; col += 1) {
				...
				// init pieces
				if (row === 0 || row === 7) {
					currRow[col] = new ChessPiece2(pieceTemplate[col], color);
				} else if (row === 1 || row === 6) {
					currRow[col] = new ChessPiece2(pawnTemplate[col], color);
				}
			}
	...
	move(moveData) {
		...
		if (moveData.castles !== '') {
			...
			if (moveData.promotesTo !== '') {
				this.tiles[to[0]][to[1]] = new ChessPiece2(
					moveData.promotesTo,
					moveData.player
				);
			}
