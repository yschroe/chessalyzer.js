...
/* harmony import */ var _GameProcessor__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./GameProcessor */ "./src/core/GameProcessor.js");
...
class Chessalyzer {
	...
	static async startBatch(
		...
		const gameProcessor = new _GameProcessor__WEBPACK_IMPORTED_MODULE_3__.default();
		...
		const t0 = perf_hooks__WEBPACK_IMPORTED_MODULE_0__.performance.now();
		...
		const t1 = perf_hooks__WEBPACK_IMPORTED_MODULE_0__.performance.now();
	...
	static async startBatchMultiCore(
		...
		const header = await _GameProcessor__WEBPACK_IMPORTED_MODULE_3__.default.processPGNMultiCore(
			path,
			cfg,
			analyzerArray,
			batchSize,
	...
	static saveData(path, data) {
		(0,fs__WEBPACK_IMPORTED_MODULE_1__.writeFile)(path, JSON.stringify(data), (err) => {
			if (err) {
				console.error(err);
				return;
			}
	...
	static loadData(path) {
		const data = JSON.parse((0,fs__WEBPACK_IMPORTED_MODULE_1__.readFileSync)(path, 'utf8'));
	...
	static generateHeatmap(data, square, fun, optData) {
		...
		if (typeof square === 'string') {
			sqrCoords = _GameProcessor__WEBPACK_IMPORTED_MODULE_3__.default.algebraicToCoords(square);
			sqrAlg = square;

			// input type [6,0]
		} else {
			sqrCoords = square;
			sqrAlg = _GameProcessor__WEBPACK_IMPORTED_MODULE_3__.default.coordsToAlgebraic(square);
		}
		...
		for (let i = 0; i < 8; i += 1) {
			...
			for (let j = 0; j < 8; j += 1) {
				...
				const loopSqrAlg = _GameProcessor__WEBPACK_IMPORTED_MODULE_3__.default.coordsToAlgebraic(
					loopSqrCoords
				);
		...
		for (let i = 0; i < map.length; i += 1) {
			for (let cnt = 0; cnt < 2; cnt += 1) {
				for (let j = 0; j < map[i].length; j += 1) {
					...
					process.stdout.write(
						chalk__WEBPACK_IMPORTED_MODULE_2___default().bgRgb(
							colorOut[0],
							colorOut[1],
							colorOut[2]
						)('    ')
...
/*! export Tracker [provided] [maybe used in chessalyzer (runtime-defined)] [usage prevents renaming] -> ./src/tracker/Tracker.js .default */
...
/* harmony export */   "Tracker": () => /* reexport safe */ _tracker_Tracker__WEBPACK_IMPORTED_MODULE_1__.default
/* harmony export */ });
