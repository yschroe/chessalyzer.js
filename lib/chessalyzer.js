...
function () {
  function BaseTracker() {
    ...
    if (this.track === undefined) {
      throw new TypeError('Your analyzer must implement a track() method!');
    }
  }
...
function () {
  function Chessalyzer() {
    ...
    this.analyzers.move.push(new _PieceTracker.default());
    this.analyzers.move.push(new _TileTracker.default());
    this.analyzers.game.push(new _GameTracker.default());
  }

  _createClass(Chessalyzer, [{
    key: "addMoveAnalyzer",
    value: function addMoveAnalyzer(analyzer) {
      this.analyzers.move.push(analyzer);
    }
  }, {
    key: "addGameAnalyzer",
    value: function addGameAnalyzer(analyzer) {
    ...
    value: function startBatch(path) {
      ...
      return new Promise(function (resolve) {
        ...
        _this.gameProcessor.processPGN(path, cfg, refreshRate, _this.analyzers).then(function () {
          ...
          _this.dataStore[0] = JSON.parse(JSON.stringify(_this.analyzers));
          resolve();
        });
      });
    }
...
function (_EventEmitter) {
  ...
  _createClass(GameProcessor, [{
    key: "processPGN",
    value: function processPGN(path, config, refreshRate, analyzers) {
      var _this2 = this;

      var cfg = GameProcessor.checkConfig(config);
      var cntGameAnalyers = analyzers.game.length;
        ...
        var processLine = function processLine(line) {
          ...
          if (line.startsWith('[') && (cfg.hasFilter || cntGameAnalyers > 0)) {
            ...
            if (cfg.filter(game) || !cfg.hasFilter) {
              _this2.processGame(game, analyzers);
            } // emit event
    ...
    key: "processGame",
    value: function processGame(game, analyzers) {
      var _this3 = this;
      ...
      for (var i = 0; i < moves.length; i += 1) {
        ...
        analyzers.move.forEach(function (a) {
      ...
      analyzers.game.forEach(function (a) {
        a.track(game);
      });
    }
  }, {
...
function (_BaseTracker) {
  ...
  function GameTracker() {
    ...
    _this = _possibleConstructorReturn(this, (GameTracker.__proto__ || Object.getPrototypeOf(GameTracker)).call(this));
    _this.wins = [0, 0, 0];
    return _this;
  }
...
function (_BaseTracker) {
  ...
  function PieceTracker() {
    ...
    _this = _possibleConstructorReturn(this, (PieceTracker.__proto__ || Object.getPrototypeOf(PieceTracker)).call(this));
    _this.b = {};
    _this.w = {}; // first layer
...
function (_BaseTracker) {
  ...
  function TileTracker() {
    ...
    _this = _possibleConstructorReturn(this, (TileTracker.__proto__ || Object.getPrototypeOf(TileTracker)).call(this));
  ...
  _createClass(TileTracker, [{
    ...
    value: function track(moveData) {
      ...
      var piece = moveData.piece;
      var takes = moveData.takes;
      var castles = moveData.castles; // move
      ...
      if (to[0] !== -1) {
        ...
        this.processMove([row, tarRookCol], player, rook); // this.processOccupation();
        // game end
      } else {// this.alivePieces = {};
          // for (let i = 0; i < 8; i += 1) {
    ...
    key: "processMove",
    value: function processMove(to, player, piece) {
      if (piece.length > 1) {
        this.tiles[to[0]][to[1]][player].movedTo += 1;
        this.tiles[to[0]][to[1]][player][piece].movedTo += 1; // this.alivePieces[player + piece] = to;
      }
    }
  }, {
    key: "processTakes",
    value: function processTakes(pos, player, takingPiece, takenPiece) {
      if (takingPiece.length > 1 && takenPiece.length > 1) {
        ...
        this.tiles[pos[0]][pos[1]][opPlayer][takenPiece].wasKilledOn += 1; // if (takenPiece.length > 1) {
