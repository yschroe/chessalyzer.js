...
var pawnTemplate$3 = ['Pa', 'Pb', 'Pc', 'Pd', 'Pe', 'Pf', 'Pg', 'Ph'];
var pieceTemplate$3 = ['Ra', 'Nb', 'Bc', 'Qd', 'Ke', 'Bf', 'Ng', 'Rh'];
    ...
    function ChessBoard() {
        ...
        for (var row = 0; row < 8; row += 1) {
            ...
            for (var col = 0; col < 8; col += 1) {
                ...
                // init pieces
                if (row === 0 || row === 7) {
                    currRow[col] = { name: pieceTemplate$3[col], color: color };
                }
                else if (row === 1 || row === 6) {
                    currRow[col] = { name: pawnTemplate$3[col], color: color };
                }
            }
...
var GameProcessor = /** @class */ (function () {
    ...
    GameProcessor.prototype.processPGN = function (path, analyzer, config, multiThreadCfg) {
        return __awaiter(this, void 0, void 0, function () {
            var readerFinished_1, isMultithreaded_1, status_1, cfg_1, forkWorker_1, games_1, game_1, lr_1, nEndForks, i, err_1;
            var _this = this;
                        ...
                        forkWorker_1 = function (games) {
                            ...
                            w.on('message', function (msg) {
                                ...
                                if (msg === 'readyForData') {
                                    w.send({
                                        games: games,
                                        analyzerData: _this.analyzerData
                                    });
                                }
                                else {
...
var Chessalyzer = /** @class */ (function () {
    function Chessalyzer() {
    }
    Chessalyzer.startBatch = function (path, analyzer, cfg, multithreadCfg) {
        if (cfg === void 0) { cfg = {}; }
        if (multithreadCfg === void 0) { multithreadCfg = { batchSize: 8000, nThreads: 1 }; }
        return __awaiter(this, void 0, void 0, function () {
            var analyzerArray, gameProcessor, t0, header, t1, tdiff, mps;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        analyzerArray = [];
                        ...
                        t0 = performance.now();
                        return [4 /*yield*/, gameProcessor.processPGN(path, analyzerArray, cfg, multithreadCfg)];
                    case 1:
                        header = _a.sent();
                        t1 = performance.now();
                        tdiff = Math.round(t1 - t0) / 1000;
                        mps = Math.round(header.cntMoves / tdiff);
                        return [2 /*return*/, __assign(__assign({}, header), { mps: mps })];
                }
            });
        });
    };
    Chessalyzer.generateHeatmap = function (data, fun, square, optData) {
        var sqrCoords = null;
        var sqrAlg = null;
    ...
    Chessalyzer.generateComparisonHeatmap = function (data1, data2, fun, square, optData) {
        var map = [];
        var max = -Infinity;
        var min = Infinity;
        // comparison heatmap
        var map0 = Chessalyzer.generateHeatmap(data1, fun, square, optData);
        var map1 = Chessalyzer.generateHeatmap(data2, fun, square, optData);
        for (var i = 0; i < 8; i += 1) {
            var dataRow = new Array(8);
            for (var j = 0; j < 8; j += 1) {
                var a = map0.map[i][j];
    ...
    Chessalyzer.printHeatmap = function (map, min, max) {
        var color = [255, 128, 0];
        var bgColor = [255, 255, 255];
        for (var i = 0; i < map.length; i += 1) {
            for (var cnt = 0; cnt < 2; cnt += 1) {
                for (var j = 0; j < map[i].length; j += 1) {
                    var alpha = max === 0 ? 0 : Math.sqrt(map[i][j] / max);
                    var colorOut = [
    ...
    Chessalyzer.getStartingPiece = function (sqr) {
        var color = null;
        var name = null;
        if (sqr !== null) {
            if (sqr[0] === 0) {
...
var TileHeatmaps = {
    TILE_OCC_ALL: {
        ...
        calc: function (data, _, loopSqrData) {
            var coords = loopSqrData.coords;
            var val = data.tiles[coords[0]][coords[1]].w.wasOn +
    ...
    TILE_OCC_BLACK: {
        ...
        calc: function (data, _, loopSqrData) {
            var coords = loopSqrData.coords;
            var val = data.tiles[coords[0]][coords[1]].b.wasOn;
            val = (val * 100) / data.cntMovesTotal;
            return val;
        }
    },
    TILE_OCC_BY_PIECE: {
        ...
        calc: function (data, sqrData, loopSqrData) {
            var sqrCoords = sqrData.coords;
            var piece = loopSqrData.piece;
            var val = 0;
            if (piece.color) {
                val =
                    data.tiles[sqrCoords[0]][sqrCoords[1]][piece.color][piece.name].wasOn;
            }
            val = (val * 100) / data.cntMovesTotal;
            return val;
        }
    ...
    TILE_KILLCOUNT: {
        ...
        calc: function (data, _, loopSqrData) {
            var coords = loopSqrData.coords;
            var val = data.tiles[coords[0]][coords[1]].b.wasKilledOn +
...
var PieceHeatmaps = {
    PIECE_KILLED_BY: {
        ...
        calc: function (data, sqrData, loopSqrData) {
            var sqrPiece = sqrData.piece;
            var loopPiece = loopSqrData.piece;
            var val = 0;
            if (sqrPiece.color &&
                loopPiece.color &&
...
var Heatmap = { Tile: TileHeatmaps, Piece: PieceHeatmaps };

export { Chessalyzer, Heatmap, Tracker };
