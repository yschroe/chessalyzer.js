...
function () {
  ...
  _createClass(ChessBoard, [{
    ...
    value: function reset() {
      this.data.cntGames += 1; // reset the pieces to default
...
function () {
  ...
  _createClass(ChessPiece, [{
    ...
    value: function killPiece(killedByPiece) {
      this.alive = false;
      this.data.cntWasKilled += 1; // if killer is not promoted pawn...

      if (!(killedByPiece.name.length === 1 || this.name.length === 1)) {
        // update killedBy of this piece
        this.stats[killedByPiece.defaultPos[0]][killedByPiece.defaultPos[1]].killedBy += 1;
      }
    }
  }, {
    key: "killedPiece",
    value: function killedPiece(_killedPiece) {
      this.data.cntHasKilled += 1; // if killer is not promoted pawn...

      if (!(_killedPiece.name.length === 1 || this.name.length === 1)) {
        // update killed stat of killer piece
        this.stats[_killedPiece.defaultPos[0]][_killedPiece.defaultPos[1]].killed += 1;
      }
    }
    /**
  ...
  _createClass(ChessTile, [{
    ...
    value: function updateStats() {
      ...
      if (this.piece.name.length !== 1) {
        this.stats[this.piece.defaultPos[0]][this.piece.defaultPos[1]].wasOnTile += 1;
      }
    }
    /**
    ...
    value: function startBatch(path) {
      ...
      return new Promise(function (resolve) {
        ...
        _this.gameProcessor.processPGN(path, cfg, refreshRate).then(function (board) {
          ...
          var mps = Math.round(dataset.data.cntMoves / tdiff);
          console.log("".concat(dataset.data.cntGames, " games (").concat(dataset.data.cntMoves, " moves) processed in ").concat(tdiff, "s (").concat(mps, " moves/s)"));

          _this.gameProcessor.reset();

          resolve(_this.dataStore[bank].data.cntGames);
        });
      });
    }
    /**
...
function (_EventEmitter) {
  ...
  _createClass(GameProcessor, [{
      ...
      return new Promise(function (resolve, reject) {
        ...
        var processLine = function processLine(line) {
          ...
          if (line.startsWith('[')) {
            ...
            game.moves = line.replace(/\{(.*?)\}\s/g, '').replace(/\d+\.+\s/g, '').replace(' *', '');

            _this2.processGame(game); // emit event


            if (_this2.board.data.cntGames % refreshRate === 0) {
              _this2.emit('status', _this2.board.data.cntGames);
            }

            if (_this2.board.data.cntGames >= cfg.cntGames) {
              lr.close();
              lr.end();
            }
    ...
    value: function checkConfig(config) {
      var cfg = {};
      cfg.filter = Object.prototype.hasOwnProperty.call(config, 'filter') ? config.filter : function () {
        return true;
      };
