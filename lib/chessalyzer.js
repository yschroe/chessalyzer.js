...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  class Chessalyzer {
    ...
    static startMultiCore(path, analyzer, cfg = {}) {
      ...
      const t0 = performance.now();
      return new Promise(resolve => {
        (0, _GameProcessorMulti.default)(path, cfg, analyzerArray).then(() => {
          const t1 = performance.now();
          const tdiff = Math.round(t1 - t0) / 1000;
      ...
      if (cluster.isMaster) {
        ...
        _GameProcessor.default.parseGames(path, config, cntGameAnalyzer).then(games => {
          ...
          for (let i = 0; i < numCPUs; i += 1) {
            ...
            console.log(`Sending Batch of size ${batchSize} to Child ${i}!`);
            workers[i].send(games.slice(i * batchSize, i * batchSize + batchSize));
