...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  class Chessalyzer {
    ...
    static startMultiCore(path, analyzer, cfg = {}, nCores = -1) {
      ...
      const t0 = performance.now();
      return new Promise(resolve => {
        (0, _GameProcessorMulti.default)(path, cfg, analyzerArray, nCores).then(() => {
          const t1 = performance.now();
          const tdiff = Math.round(t1 - t0) / 1000;
  ...
  const processMulti = (path, cfg, analyzer, nCoresTar) => {
    ...
    return new Promise(resolve => {
      ...
      if (cluster.isMaster) {
        ...
        const config = _GameProcessor.default.checkConfig(cfg);
        ...
        _GameProcessor.default.parseGames(path, config, cntGameAnalyzer).then(games => {
          ...
          for (let i = 0; i < nCores; i += 1) {
            ...
            console.log(`Sending Batch of size ${batchSize} to Child ${i}!`); // send batch to worker
