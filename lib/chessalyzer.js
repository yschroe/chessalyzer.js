...
function () {
  function Chessalyzer() {
    ...
    this.gameProcessor = new _GameProcessor.default(); // this.analyzers = { move: [], game: [] };
  ...
  _createClass(Chessalyzer, [{
    ...
    key: "getGameTracker",
    value: function getGameTracker() {
      return new _GameTracker.default();
    }
  }, {
    key: "getPieceTracker",
    value: function getPieceTracker() {
      return new _PieceTracker.default();
    }
  }, {
    key: "getTileTracker",
    value: function getTileTracker() {
      return new _TileTracker.default();
    }
    /**
    ...
    value: function startBatch(path) {
      ...
      var analyzers = arguments[2];
      var refreshRate = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 250;
      var gameProcessor = new _GameProcessor.default();
      return new Promise(function (resolve) {
        var t0 = performance.now();
        gameProcessor.processPGN(path, cfg, analyzers, refreshRate).then(function () {
          var t1 = performance.now();
          var tdiff = Math.round(t1 - t0) / 1000;
          var mps = Math.round(gameProcessor.cntMoves / tdiff);
          console.log("".concat(gameProcessor.cntGames, " games (").concat(gameProcessor.cntMoves, " moves) processed in ").concat(tdiff, "s (").concat(mps, " moves/s)")); // this.gameProcessor.reset();
