...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  class Chessalyzer {
    ...
    static async startBatchMultiCore(path, analyzer, cfg = {}) {
      ...
      const t0 = performance.now();
      const header = await _GameProcessor.default.processPGNMultiCore(path, cfg, analyzerArray);
      const t1 = performance.now();
      const tdiff = Math.round(t1 - t0) / 1000;
      const mps = Math.round(header.cntMoves / tdiff);
      console.log(`${header.cntGames} games (${header.cntMoves} moves) processed in ${tdiff}s (${mps} moves/s)`);
      ...
      return new Promise(resolve => {
        ...
        if (cluster.isMaster) {
          ...
          const processLine = line => {
            ...
            if (line.startsWith('[') && (cfg.hasFilter || cntGameAnalyzer > 0)) {
              ...
              if (cfg.filter(game) || !cfg.hasFilter) {
                ...
                if (cntGames % 10000 === 0) {
                  console.log(`Forking! ${Object.keys(cluster.workers).length}`);
                  const w = cluster.fork();
                  w.send(games);
                  games = []; // on worker finish
          ...
          process.on('message', msg => {
            const t0 = performance.now(); // create new GameProcessor object and attach analyzers
            ...
            console.log(`Worker: ${proc.cntGames} games (${proc.cntMoves} moves) processed in ${tdiff}s (${mps} moves/s)`); // send result of batch to master
