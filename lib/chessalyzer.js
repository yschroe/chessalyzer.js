...
function (_EventEmitter) {
  ...
  function GameProcessor() {
    ...
    _this.tileTracker = new _TileTracker.default();
    _this.gameTracker = new _GameTracker.default();
  ...
  _createClass(GameProcessor, [{
    ...
    value: function processPGN(path, config, refreshRate) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        lr.on('end', function () {
          console.log('Read entire file.'); // console.log(this.pieceTracker.pieces.w.Pe);
          // console.log(this.tileTracker.tiles);

          console.log(_this2.gameTracker.wins);
          resolve();
        });
      });
    }
    ...
    value: function processGame(game) {
      ...
      for (var i = 0; i < moves.length; i += 1) {
        ...
        this.parseMove(moves[i]); // ___ PLACE MOVE-ANALYZERS HERE ___

        this.pieceTracker.track(this.currentMove);
        this.tileTracker.track(this.currentMove); // ___ END
      ...
      this.cntGames += 1;
      this.board.reset(); // ___ PLACE GAME-ANALYZERS HERE ___

      this.gameTracker.track(game); // ___ END
    }
  }, {
    key: "reset",
    value: function reset() {
...
function () {
  function PieceTracker() {
    ...
    Object.keys(this.pieces.w).forEach(function (key) {
      pawnTemplate.forEach(function (val) {
