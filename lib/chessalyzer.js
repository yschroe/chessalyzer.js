...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  class Chessalyzer {
    ...
    static startBatchMultiCore(path, analyzer, cfg = {}, nCores = -1) {
      ...
      const t0 = performance.now();
      return new Promise(resolve => {
        _GameProcessor.default.processPGNMultiCore(path, cfg, analyzerArray, nCores).then(header => {
          const t1 = performance.now();
          const tdiff = Math.round(t1 - t0) / 1000;
          const mps = Math.round(header.cntMoves / tdiff);
          console.log(`${header.cntGames} games (${header.cntMoves} moves) processed in ${tdiff}s (${mps} moves/s)`);
    ...
    static processPGNMultiCore(path, cfg, analyzer, nCoresTar) {
      return new Promise(resolve => {
        ...
        if (cluster.isMaster) {
          ...
          const config = GameProcessor.checkConfig(cfg); // Parse games first

          GameProcessor.parseGames(path, config, cntGameAnalyzer).then(games => {
            ...
            function addTrackerData(gameTracker, moveTracker, nGames, nMoves) {
            ...
            for (let i = 0; i < nCores; i += 1) {
              const w = cluster.fork(); // send batch to worker

              w.send(games.slice(i * batchSize, i * batchSize + batchSize)); // on worker finish
