...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  class Chessalyzer {
    ...
    static async startBatchMultiCore(path, analyzer, cfg = {}, batchSize = 6000, nThreads = 2, customPath = '') {
      // check if single analyzer or array is passed
      let analyzerArray = analyzer;
      ...
      const t0 = performance.now();
      const header = await _GameProcessor.default.processPGNMultiCore(path, cfg, analyzerArray, batchSize, nThreads, customPath);
      const t1 = performance.now();
      const tdiff = Math.round(t1 - t0) / 1000;
      const mps = Math.round(header.cntMoves / tdiff);
      console.log(`${header.cntGames} games (${header.cntMoves} moves) processed in ${tdiff}s (${mps} moves/s)`);
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  class GameProcessor extends EventEmitter {
    ...
    static processPGNMultiCore(path, config, analyzer, batchSize, nThreads, customPath) {
      return new Promise(resolve => {
        ...
        let cntGames = 0;
        let cntMoves = 0;
        let readerFinished = false; // eslint-disable-next-line no-undef
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;(function (global, factory) {
  ...
  process.on('message', msg => {
    const proc = new _GameProcessor.default(); // merge available Trackers
